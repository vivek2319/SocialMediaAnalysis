...
edges <- getEdges(data = new_tw_df, tweets = "text", source = "screenName", "retweetCount", "favorited", "topic_col", "created", str.length = 20)
nodes <- getNodes(edges)
g <- graph.data.frame(edges, directed = TRUE, vertices = nodes)
write.graph(g, "gephi_topics3.graphml", format="graphml")



## Write to CSV for Tableau
write.table(new_tw_df, "topic_col3.csv", sep=",", col.names=T)
