...
tweets <- searchTwitter("putin", n=3000, lang="en")
tw_df <- twListToDF(tweets)

# Labeling our network
classified = cbind(tw_df, rep("putin", nrow(tw_df)))
colnames(classified)[ncol(classified)]="classifier"

tw_df = classified

tweets2 <- searchTwitter("trump", n=3000)
tw_df_2 <- twListToDF(tweets2)


# Labeling our network
classified = cbind(tw_df_2, rep("trump", nrow(tw_df_2)))
colnames(classified)[ncol(classified)]="classifier"
...
tweets3 <- searchTwitter("shinzo abe", n=3000)
tw_df_3 <- twListToDF(tweets3)


# Labeling our network
classified = cbind(tw_df_3, rep("abe", nrow(tw_df_3)))
colnames(classified)[ncol(classified)]="classifier"
...
tweets4 <- searchTwitter("moon jae", n=3000)
tw_df_4 <- twListToDF(tweets4)


# Labeling our network
classified = cbind(tw_df_4, rep("moon", nrow(tw_df_4)))
colnames(classified)[ncol(classified)]="classifier"

tw_df_4 = classified


tweets5 <- searchTwitter("kim jong", n=3000)
tw_df_5 <- twListToDF(tweets5)


# Labeling our network
classified = cbind(tw_df_5, rep("kim jong", nrow(tw_df_5)))
colnames(classified)[ncol(classified)]="classifier"
...
tweets6 <- searchTwitter("xi jinping", n=3000)
tw_df_6 <- twListToDF(tweets6)


# Labeling our network
classified = cbind(tw_df_6, rep("jinping", nrow(tw_df_6)))
colnames(classified)[ncol(classified)]="classifier"
...
total_en = rbind.fill(total_en, tw_df_3)
total_en = rbind.fill(total_en, tw_df_4)
total_en = rbind.fill(total_en, tw_df_5)
total_en = rbind.fill(total_en, tw_df_6)
...
write.graph(g, file="multiple searches graph.graphml", format="graphml")

getwd()

write.csv(total_en_final, "multiple searches tableau.csv", row.names = FALSE)
